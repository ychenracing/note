...
2. `2>&1`  
  2表示标准错误输出，&表示等同于。这条命令表示标准错误输出的重定向等同于标准输出。一般和1组合起来使用，组合使用时，由于标准输出重定向到了空设备文件，则标准错误输出通过标准输出也重定向到了空设备文件。如`java Test >/dev/null 2>&1`。

()用作命令组时，其内的命令组在子shell中执行，其中的cd重定向工作目录不会影响括号外的那些命令。()命令组执行完毕之后，脚本才会继续执行。
...
2. **命令替换（执行一条命令获取其结果）**  
   \$()等同于\`cmd\`，shell扫描一遍命令行，发现了\$(cmd)结构，便将\$(cmd)中的cmd执行一次，得到其标准输出，再将此输出放到原来位置。有些shell不支持，如tcsh。  
   如
...
所有的UNIX命令，要取结果或输出，都要使用$()或``。

括号前加上`$`符号表明取括号中的返回值。比如`echo $(expr 3 + 1)`，括号中的命令返回值为表达式的结果`4`，用`$`取出这个返回值；`echo $((1 < 5))`括号中的返回值为`1`，用`$`取出这个返回值；`echo $((3 - 1))`括号中的返回值为`2`，用`$`取出这个返回值。
...
1. **整数扩展**  
  ...
  - 如果表达式的结果为0，那么返回“0”值（返回值0表示“假”），**返回的退出状态码为1**。如`echo $((1 - 1))`输出为`0`，用在`if ((1 - 1))`时为假，会执行else的内容。
  - 如果表达式的结果为非0，那么返回表达式结果值（返回值非0表示“真”），**返回的退出状态码为0**。如`echo $((3 - 1))`输出为`2`，用在`if ((3 - 1))`时为真，会执行then的内容。
